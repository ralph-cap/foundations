...
export class RouterInspector {
  ...
  currentLocationToBe(expectedLocationPath: string): boolean {
    return !!this.routerLocation && this.routerLocation.pathname === expectedLocationPath;
  }
}
...
describe('BookOverviewRoute', () => {
  ...
  it('redirects to BookDetails upon click on a row', () => {
    ...
    return bookServiceMockPromise.then(() => { // make sure assertions are done AFTER the state is updated
      ...
      // then
      expect(routeInspector.currentLocationToBe(`/book-app/book/${books[0].id}`)).toBeTruthy();
    });
  });
});
