...
class ExistingBookDetails extends Component<ExistingBookDetailsProps, ExistingBookDetailsState> {
  state = {
    book: null
  };
  ...
  render() {
    if (this.state.book) {
      return <BookDetails book={this.state.book!}
                          onBookChange={book => this.props.bookService.save(book)
...
export const BookDetailsRoute = (props: BookDetailsRouteProps) => {
  ...
  return <Route path="/book-app/book"
                render={({match}) => (
                  <Switch>
                    <Route exact
                           path={match.path}
                           render={props => <BookDetails {...props}
                                                         book={{title: '', authors: ''}}
                                                         onBookChange={book => bookService.save(book).then(
                                                           () => props.history.push('/book-app/books'))}/>}
                    />
                    <Route path={`${match.path}/:id`}
                           render={props => <ExistingBookDetails {...props}
